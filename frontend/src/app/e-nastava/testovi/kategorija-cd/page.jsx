import Link from "next/link";
import Section from "@/components/Section";
import BlackButton from "@/components/buttons/BlackButton";

export default function KategorijaCD() {
  const buttons = ["ISPIT 14", "ISPIT 15", "ISPIT 16"];

  return (
    <div className="flex flex-col gap-16">
      <div className="flex flex-col items-start gap-6">
        <Section number={"C-D"} text={"kategorija"} />
        <p className="text-justify ">
          Ovdje se nalaze
          <strong> dodatna pitanja za testove kategorije C i D</strong> za
          polaganje vozačkog ispita propisa o sigurnosti prometa na cestama.
          Svako pitanje može imati jedan ili više točnih odgovora.
        </p>
      </div>
      <div className="grid grid-cols-6 max-xl:grid-cols-5 max-[1400px]:grid-cols-4 max-[1120px]:grid-cols-3 max-[680px]:grid-cols-2 max-[480px]:grid-cols-1 gap-y-10 max-[1400px]:gap-y-5 ml-4 ">
        {buttons.map((description, index) => (
          <Link
            key={index}
            href={`/e-nastava/testovi/kategorija-cd/${description
              .toLowerCase()
              .replace(" ", "-")}`}
          >
            <BlackButton text={description} />
          </Link>
        ))}
      </div>
    </div>
  );
}
