import { button } from "@nextui-org/theme";

export default function BlackButton({ text }) {
  return (
    <button className="relative group w-[200px] h-16 border border-black-40 bg-black-40 text-white-60 overflow-hidden">
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#BE1E2D,#231F20)] opacity-0 group-hover:opacity-100 transition-opacity duration-400 ease-in-out"></div>
      <span className="relative text-base font-light text-center">{text}</span>
    </button>
  );
}
